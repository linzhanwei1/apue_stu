...
static void alrm_action(int s, siginfo_t *infop, void *unused)
{
    ...
    for (int i = 0; i < JOB_MAX; i++)
    {
        ...
        {
            job[i]->time_remian--;
            if (job[i]->time_remian == 0)
            {
                job[i]->jobp(job[i]->arg); // 时间误差，函数处理时间会影响计时
                if (job[i]->repeat == 1)
                {
                    job[i]->time_remian = job[i]->sec;
                }
                else
